  ...
  class JwkKey
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TimeLogic::DirectApi::JwkKey` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TimeLogic::DirectApi::JwkKey`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      kty_validator = EnumAttributeValidator.new('String', ["OKP"])
      return false unless kty_validator.valid?(@kty)
      return false if @crv.nil?
      crv_validator = EnumAttributeValidator.new('String', ["Ed25519"])
      return false unless crv_validator.valid?(@crv)
      return false if @alg.nil?
      alg_validator = EnumAttributeValidator.new('String', ["EdDSA"])
      return false unless alg_validator.valid?(@alg)
      return false if @use.nil?
      use_validator = EnumAttributeValidator.new('String', ["sig"])
    ...
    def kty=(kty)
      validator = EnumAttributeValidator.new('String', ["OKP"])
    ...
    def crv=(crv)
      validator = EnumAttributeValidator.new('String', ["Ed25519"])
    ...
    def alg=(alg)
      validator = EnumAttributeValidator.new('String', ["EdDSA"])
    ...
    def use=(use)
      validator = EnumAttributeValidator.new('String', ["sig"])
