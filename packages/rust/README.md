# Rust API client for timelogic-api
...
Put the package under your project folder in a directory named `timelogic-api` and add the following to `Cargo.toml` under `[dependencies]`:

```
timelogic-api = { path = "./timelogic-api" }
```
