/*
TimeLogic API | A World Time API

TimeLogic API | A World Time API. World time, timezone, calendar, duration, and signed response operations.

API version: 1.0.0
*/

// Code generated by TimeLogic API SDK generator (https://github.com/TimeLogic-API-LLC/timelogic-api-sdks.git); DO NOT EDIT.

package timelogicapi

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the DstResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DstResponse{}

// DstResponse struct for DstResponse
type DstResponse struct {
	Unix int64 `json:"unix"`
	UnixMs int64 `json:"unix_ms"`
	Utc time.Time `json:"utc"`
	IsoLocal NullableString `json:"iso_local"`
	Rfc2822 string `json:"rfc2822"`
	Human string `json:"human"`
	DayNumber int32 `json:"day_number"`
	DayShort string `json:"day_short"`
	DayFull string `json:"day_full"`
	Timezone NullableString `json:"timezone"`
	Formatted *string `json:"formatted,omitempty"`
	DstActive bool `json:"dst_active"`
	NextTransition NullableTime `json:"next_transition,omitempty"`
}

type _DstResponse DstResponse

// NewDstResponse instantiates a new DstResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDstResponse(unix int64, unixMs int64, utc time.Time, isoLocal NullableString, rfc2822 string, human string, dayNumber int32, dayShort string, dayFull string, timezone NullableString, dstActive bool) *DstResponse {
	this := DstResponse{}
	this.Unix = unix
	this.UnixMs = unixMs
	this.Utc = utc
	this.IsoLocal = isoLocal
	this.Rfc2822 = rfc2822
	this.Human = human
	this.DayNumber = dayNumber
	this.DayShort = dayShort
	this.DayFull = dayFull
	this.Timezone = timezone
	this.DstActive = dstActive
	return &this
}

// NewDstResponseWithDefaults instantiates a new DstResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDstResponseWithDefaults() *DstResponse {
	this := DstResponse{}
	return &this
}

// GetUnix returns the Unix field value
func (o *DstResponse) GetUnix() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Unix
}

// GetUnixOk returns a tuple with the Unix field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetUnixOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Unix, true
}

// SetUnix sets field value
func (o *DstResponse) SetUnix(v int64) {
	o.Unix = v
}

// GetUnixMs returns the UnixMs field value
func (o *DstResponse) GetUnixMs() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.UnixMs
}

// GetUnixMsOk returns a tuple with the UnixMs field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetUnixMsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UnixMs, true
}

// SetUnixMs sets field value
func (o *DstResponse) SetUnixMs(v int64) {
	o.UnixMs = v
}

// GetUtc returns the Utc field value
func (o *DstResponse) GetUtc() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.Utc
}

// GetUtcOk returns a tuple with the Utc field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetUtcOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Utc, true
}

// SetUtc sets field value
func (o *DstResponse) SetUtc(v time.Time) {
	o.Utc = v
}

// GetIsoLocal returns the IsoLocal field value
// If the value is explicit nil, the zero value for string will be returned
func (o *DstResponse) GetIsoLocal() string {
	if o == nil || o.IsoLocal.Get() == nil {
		var ret string
		return ret
	}

	return *o.IsoLocal.Get()
}

// GetIsoLocalOk returns a tuple with the IsoLocal field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *DstResponse) GetIsoLocalOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.IsoLocal.Get(), o.IsoLocal.IsSet()
}

// SetIsoLocal sets field value
func (o *DstResponse) SetIsoLocal(v string) {
	o.IsoLocal.Set(&v)
}

// GetRfc2822 returns the Rfc2822 field value
func (o *DstResponse) GetRfc2822() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Rfc2822
}

// GetRfc2822Ok returns a tuple with the Rfc2822 field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetRfc2822Ok() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Rfc2822, true
}

// SetRfc2822 sets field value
func (o *DstResponse) SetRfc2822(v string) {
	o.Rfc2822 = v
}

// GetHuman returns the Human field value
func (o *DstResponse) GetHuman() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Human
}

// GetHumanOk returns a tuple with the Human field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetHumanOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Human, true
}

// SetHuman sets field value
func (o *DstResponse) SetHuman(v string) {
	o.Human = v
}

// GetDayNumber returns the DayNumber field value
func (o *DstResponse) GetDayNumber() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.DayNumber
}

// GetDayNumberOk returns a tuple with the DayNumber field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetDayNumberOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DayNumber, true
}

// SetDayNumber sets field value
func (o *DstResponse) SetDayNumber(v int32) {
	o.DayNumber = v
}

// GetDayShort returns the DayShort field value
func (o *DstResponse) GetDayShort() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DayShort
}

// GetDayShortOk returns a tuple with the DayShort field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetDayShortOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DayShort, true
}

// SetDayShort sets field value
func (o *DstResponse) SetDayShort(v string) {
	o.DayShort = v
}

// GetDayFull returns the DayFull field value
func (o *DstResponse) GetDayFull() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DayFull
}

// GetDayFullOk returns a tuple with the DayFull field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetDayFullOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DayFull, true
}

// SetDayFull sets field value
func (o *DstResponse) SetDayFull(v string) {
	o.DayFull = v
}

// GetTimezone returns the Timezone field value
// If the value is explicit nil, the zero value for string will be returned
func (o *DstResponse) GetTimezone() string {
	if o == nil || o.Timezone.Get() == nil {
		var ret string
		return ret
	}

	return *o.Timezone.Get()
}

// GetTimezoneOk returns a tuple with the Timezone field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *DstResponse) GetTimezoneOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Timezone.Get(), o.Timezone.IsSet()
}

// SetTimezone sets field value
func (o *DstResponse) SetTimezone(v string) {
	o.Timezone.Set(&v)
}

// GetFormatted returns the Formatted field value if set, zero value otherwise.
func (o *DstResponse) GetFormatted() string {
	if o == nil || IsNil(o.Formatted) {
		var ret string
		return ret
	}
	return *o.Formatted
}

// GetFormattedOk returns a tuple with the Formatted field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DstResponse) GetFormattedOk() (*string, bool) {
	if o == nil || IsNil(o.Formatted) {
		return nil, false
	}
	return o.Formatted, true
}

// HasFormatted returns a boolean if a field has been set.
func (o *DstResponse) HasFormatted() bool {
	if o != nil && !IsNil(o.Formatted) {
		return true
	}

	return false
}

// SetFormatted gets a reference to the given string and assigns it to the Formatted field.
func (o *DstResponse) SetFormatted(v string) {
	o.Formatted = &v
}

// GetDstActive returns the DstActive field value
func (o *DstResponse) GetDstActive() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.DstActive
}

// GetDstActiveOk returns a tuple with the DstActive field value
// and a boolean to check if the value has been set.
func (o *DstResponse) GetDstActiveOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DstActive, true
}

// SetDstActive sets field value
func (o *DstResponse) SetDstActive(v bool) {
	o.DstActive = v
}

// GetNextTransition returns the NextTransition field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *DstResponse) GetNextTransition() time.Time {
	if o == nil || IsNil(o.NextTransition.Get()) {
		var ret time.Time
		return ret
	}
	return *o.NextTransition.Get()
}

// GetNextTransitionOk returns a tuple with the NextTransition field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *DstResponse) GetNextTransitionOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.NextTransition.Get(), o.NextTransition.IsSet()
}

// HasNextTransition returns a boolean if a field has been set.
func (o *DstResponse) HasNextTransition() bool {
	if o != nil && o.NextTransition.IsSet() {
		return true
	}

	return false
}

// SetNextTransition gets a reference to the given NullableTime and assigns it to the NextTransition field.
func (o *DstResponse) SetNextTransition(v time.Time) {
	o.NextTransition.Set(&v)
}
// SetNextTransitionNil sets the value for NextTransition to be an explicit nil
func (o *DstResponse) SetNextTransitionNil() {
	o.NextTransition.Set(nil)
}

// UnsetNextTransition ensures that no value is present for NextTransition, not even an explicit nil
func (o *DstResponse) UnsetNextTransition() {
	o.NextTransition.Unset()
}

func (o DstResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DstResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["unix"] = o.Unix
	toSerialize["unix_ms"] = o.UnixMs
	toSerialize["utc"] = o.Utc
	toSerialize["iso_local"] = o.IsoLocal.Get()
	toSerialize["rfc2822"] = o.Rfc2822
	toSerialize["human"] = o.Human
	toSerialize["day_number"] = o.DayNumber
	toSerialize["day_short"] = o.DayShort
	toSerialize["day_full"] = o.DayFull
	toSerialize["timezone"] = o.Timezone.Get()
	if !IsNil(o.Formatted) {
		toSerialize["formatted"] = o.Formatted
	}
	toSerialize["dst_active"] = o.DstActive
	if o.NextTransition.IsSet() {
		toSerialize["next_transition"] = o.NextTransition.Get()
	}
	return toSerialize, nil
}

func (o *DstResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"unix",
		"unix_ms",
		"utc",
		"iso_local",
		"rfc2822",
		"human",
		"day_number",
		"day_short",
		"day_full",
		"timezone",
		"dst_active",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varDstResponse := _DstResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varDstResponse)

	if err != nil {
		return err
	}

	*o = DstResponse(varDstResponse)

	return err
}

type NullableDstResponse struct {
	value *DstResponse
	isSet bool
}

func (v NullableDstResponse) Get() *DstResponse {
	return v.value
}

func (v *NullableDstResponse) Set(val *DstResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableDstResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableDstResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDstResponse(val *DstResponse) *NullableDstResponse {
	return &NullableDstResponse{value: val, isSet: true}
}

func (v NullableDstResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDstResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


